...
class SoundController extends Controller
{
    ...
    {        
        $data['sounds'] = \App\Albummedia::where('category_id',$category_id)->where('lang',\App::getLocale())->where('type','SOUND')->orderby('id','DESC')->get();
        $data['category_id'] = $category_id;
        return view('admin.sounds.index')->with($data);
    }
    ...
    {            
        ...
        if (\Request::isMethod('post')) {           
            ...
            if(\Input::hasFile('infile')) {
                ...
                $fid = time();
                \Input::file('infile')->move('uploads/sounds', $fid.'.'.$extension);
            ...
            return \Redirect::to(config('app.prefix','Cpanel').'/sounds/'.$category_id)->with(['success'=>trans('cp.oprationsuccess')]);
        }else{
            $data['type'] = 'add';
            return view('admin.sounds.form')->with($data);
        } 
    }
    ...
    {          
        ...
        if (\Request::isMethod('post')) {
            ...
            if(\Input::hasFile('infile')) {
                ...
                $sound =  \App\Albummedia::find($id);

                try{                    
                    @unlink('uploads/sounds/'.$sound->infile);
                }catch (Exception $exp){}
            ...
            $data['type'] = 'edit';
            $data['sound'] = \App\Albummedia::find($id);
            return view('admin.sounds.form')->with($data);      
        }
